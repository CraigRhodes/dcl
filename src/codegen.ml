...
let translate (globals, functions) =
  ...
  let cmpstr_t = L.function_type i32_t [| L.pointer_type i8_t ; L.pointer_type i8_t |] in
  let cmpstr_func = L.declare_function "strcmp" cmpstr_t the_module in
  ...
  in StringMap.add n local_var m in
    ...
    | A.Equal     -> if      L.type_of e1' == ltype_of_typ (A.Simple(A.Int))    then L.build_icmp L.Icmp.Eq e1' e2' "tmp" builder
                       ...
                       let result = L.build_call cmpstr_func [| str_ptr_e1' ; str_ptr_e2' |] "tmp" builder in
